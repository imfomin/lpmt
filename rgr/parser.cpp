...
enum class ObjectType { Variable, Constant }; 
...
std::ostream& operator <<(std::ostream& stream, ObjectName object_name) {
	...
	case ObjectType::Constant: stream << "Константа: "  << *(    (int*)      object_name.name_pointer); break;
	...
	*  а затем передаёт указатель на неё, поэтому необходимо эту память освободить в деструкторе данного класса */
	~ParsedProgram() {
		for (auto& name : name_table) {
			...
			case ObjectType::Constant: delete (int*)name.name_pointer; 	   break;
			}
		}
	}
...
private:
	...
	*/
	void add_constant() {
		for(int i = 0; i < name_table.size(); ++i) {
			if (name_table[i].type != ObjectType::Constant) continue;
		...
		name_table.push_back(ObjectName(ObjectType::Constant, (new_constant)));
